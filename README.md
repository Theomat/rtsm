...
```bash
python -m rtsm ./examples/humaneval_pass200.csv
```
...
The ```SAT20-MAIN.csv`` contains 400 tests with none that can be at first glance deemed necessary making it a much arder problem so we will use multiple CPUs:

```bash
python -m rtsm.check_solution ./examples/SAT20-MAIN.csv  -p 8
```
...
```bash
python -m rtsm.check_solution ./examples/x264_etime.csv  -p 8 --splits 14
```
